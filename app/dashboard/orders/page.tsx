    ...
    <div className="space-y-6">
      ...
      <form className="flex flex-col md:flex-row gap-3">
        ...
        <select name="status" defaultValue={sp.status ?? 'any'}
          ...
          {STATUSES.map(s => <option key={s} value={s}>{s === 'any' ? 'All Status' : s.charAt(0).toUpperCase() + s.slice(1)}</option>)}
        </select>
              ...
              {orders.length === 0
                ...
                : orders.map((o) => (
                  <tr key={o.id} className="hover:bg-gray-50 transition-colors">
                    <td className="px-5 py-4 font-medium text-sky-700">#{o.id}</td>
        ...
        {totalPages > 1 && (
          <div className="px-6 py-4 border-t border-gray-100 flex items-center justify-between">
            ...
            <div className="flex gap-2">
              {page > 1 && <Link href={`?page=${page - 1}&status=${sp.status ?? ''}`} className="px-4 py-2 text-sm border border-gray-200 rounded-xl hover:bg-gray-50">← Prev</Link>}
              {page < totalPages && <Link href={`?page=${page + 1}&status=${sp.status ?? ''}`} className="px-4 py-2 text-sm bg-sky-700 text-white rounded-xl hover:bg-sky-800">Next →</Link>}
            </div>
          </div>
        )}
      </div>
